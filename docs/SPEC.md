...
- the agent produces both a human-executable MoP and an LLM/agent-readable Markdown installation guide;
- Codex integration is through an on-demand MCP server that supports iterative refinement;
- standalone mode is triggered through REST, uses LangChain with GPT-4.1 mini or equivalent configured model, and uses agentic memory through LangMem and backing stores;
...
- Agent-readable installation guide contract.
